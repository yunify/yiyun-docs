...
AppCenter 的 metadata service 是在存储服务存储服务存储服务etcd 基础之上进行了二次开发，主要增加了 self 属性，即每个节点只能从该服务获取到自身相关的信息，如本机 IP、server ID 等，
...
mac|节点 mac 地址
sid|节点 server ID，亿栖云调度系统自动为每个节点分配的从1开始的整数。
gid|节点 group ID，亿栖云调度系统自动为每个组分配的从1开始的整数，每一个节点和它的 replica 组成一个 group，即它们的 gid 相同，这个是为分片式分布式系统(多主多从，每个主和它的从为同一个组)提供的特性。
gsid|节点 global server ID，亿栖云调度系统自动为每个节点分配的全球唯一的9位随机整数 ID。
node_id|节点 node ID，亿栖云调度系统自动为每个节点分配的节点 ID，是一个以 cln- 开头的唯一标识，此 ID 不会变更。
instance_id|节点云服务器 ID，亿栖云调度系统自动为每个节点分配的云服务器 ID，是一个以i-开头的唯一标识，此 ID 是云服务器的 hostname，每次启动都会变更，如关闭集群然后启动集群，该节点 instance ID 可能会变更。
cpu|节点 CPU 核数
...
| - | - |
app_id|集群所属的应用 ID
cluster_id|集群 ID，用户在创建应用的时候亿栖云调度系统自动生成的一个以 cl- 开头的唯一标识，如 cl-0u0a6u1j。
...
### env
  ...
  对于参数类型是 accesskey 的参数，在其原来目录下，亿栖云调度系统会添加 /access_key_id 和 /secret_access_key 两个子目录，分别存储 API 密钥ID 和私钥。
### adding-hosts
...
### cmd
  ...
  cmd 表示本节点需要执行的命令。开发者不需要用到这类信息，这是亿栖云调度系统转发并执行应用命令，如启动应用命令等。
...
在创建好一个集群后，登陆到任意一个节点，在文件 /etc/confd/confd.toml 里找到 nodes 这一行(这个文件是亿栖云调度系统在用户创建集群的时候自动生成的)，这一行定义的是 metadata server 的 IP 地址，任取一个 IP，运行下面命令即可看到所有信息。注明：同一 VPC 里所有集群这个文件内容相同。

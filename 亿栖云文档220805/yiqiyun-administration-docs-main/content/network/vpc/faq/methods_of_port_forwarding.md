...
另外需要放行vpc所在安全组源端口对应的下行规则，并应用修改安全组，如图
...
> **注意**
>
> 1.可以针对同一个内网云服务器的内网端口添加多条转发规则，只要采用不同的源端口即可，添加完成后需要在 VPC 详情页点击**应用修改**。
>
> 2.可以针对同一个内网云服务器的多个内网服务添加多条转发规则。
>
> 3.如果想实现指定 IP 或者 IP 段访问源端口的服务，可以在安全组的源 IP 处进行配置。（相当于白名单）如下图所示。
...
比如需要本地通过公网访问数据库服务，数据库节点的ip地址为:192.168.10.2，就需要配置一条端口转发的规则将数据库服务的3306端口暴露给公网（这种做法是有风险的，容易被攻击，建议加上源ip）。

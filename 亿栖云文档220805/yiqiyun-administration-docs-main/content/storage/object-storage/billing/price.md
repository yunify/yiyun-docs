...
- **取回费用** 指的是亿栖云对象存储将数据从低频存储取回到标准存储的费用。

## 免费额度

注册成功并完成认证的的亿栖云对象存储用户，亿栖云为您提供一定额度的亿栖云对象存储免费使用套餐。免费政策从用户创建 Bucket 之日算起，在未来 12 个月免费赠送:
...
若当月费用扣除后余额不足，则用户的所有 Bucket 将会处于 **暂停** 状态，此时用户无法使用亿栖云对象存储的任何功能。等到用户充值足够的金额后，用户的所有 Bucket 将会自动恢复。

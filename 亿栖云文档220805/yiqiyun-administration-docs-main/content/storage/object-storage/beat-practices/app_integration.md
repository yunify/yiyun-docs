...
使用亿栖云对象存储之前，用户需创建相应的 Bucket，用于储存 Object。详细操作可参考 [创建 Bucket](/storage/object-storage/manual/console/bucket_manage/basic_opt/#创建-bucket)。
...
4. 最后，输入以下命令来安装亿栖云对象存储 SDK 的依赖包
...
4. 打开 BucketListController，在 viewDidLoad 方法里面初始化我们的亿栖云对象存储服务：
...
5. 为了灵活使用，除了上述初始化亿栖云对象存储服务的方法之外，SDK 也支持在服务初始化的时候传入配置信息：
...
亿栖云对象存储提供了一个签名服务器的 [样例](https://github.com/yunify/qingstor-demo-auth-server) 供 App 开发者参考。网页端使用 JavaScript SDK 配合签名服务器进行上传可以参考官方提供的 [Demo 项目](https://github.com/yunify/qingstor-sdk-js-demo)。
...
3. 将该结果附加到请求头中，作为最终发送给亿栖云对象存储的请求
...
3. 用户通过表单上传文件以及 Signature 给亿栖云对象存储。表单上传的具体方法，请见 [表单上传](/storage/object-storage/api/object/post/)

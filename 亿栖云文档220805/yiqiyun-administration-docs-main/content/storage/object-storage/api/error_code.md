...
| internal_error | 亿栖云对象存储系统内部错误，请稍后重试 | 500 |
| service_unavailable | 亿栖云对象存储系统临时不可用，请稍后重试 | 503 |
...
- 单个 Object 上传时，亿栖云对象存储对其上限做出了限制，最大为 5G。
...
```json
{
  ...
  "url": "http://docsv3.yiqiyun.com/api/error_code.html"
}
```
...
| code | 错误码 |
| message | 详细错误信息 |
| request_id | 服务端为该次错误请求生成的唯一标示。亿栖云对象存储研发通过该 ID，能更快定位问题。 |
| url | 指向相关帮助文档 |

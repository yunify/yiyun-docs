...
- 亿栖云对象存储定义事件通知为 Bucket 的子资源，因此，只有 Bucket 的所有者才能调用该 API。
- 若设置多个通知规则，亿栖云对象存储会对每个规则逐一进行检查匹配，将可能触发多个事件。
...
| notify_url | String  | 接受通知事件处理结果的 URL。当事件处理完成后，其结果将会以 POST 方式向该 URL 发送。若 POST 超时，亿栖云对象存储将会重试。目前超时时间为 5s，重试间隔为 1s。| 否 |

**说明**

当用户设置 `cloudfunc` 为 `image` 时，`cloudfunc_args` 参数有效，其键值说明如下。亿栖云对象存储将按照指定的图片处理规则对图片进行处理，并将处理结果另存回亿栖云对象存储的 Bucket。

| 名称 | 类型 | 说明 | 是否必须 |
| - | - | - | - |
| action | String | 图片的具体操作参数, 见 [图片基本处理服务](/storage/object-storage/manual/console/data_process/image_process/) | 是 |
| key_prefix | String | 处理结果存回亿栖云对象存储的 Bucket 的对象名称前缀，默认为 `gen` | 否 |
| key_seprate | String | 分隔符，默认为 `_`。处理结果的对象名为： `key_prefix` + `key_seprate` + `origin_object` | 否 |
| save_bucket | String | 处理结果存回亿栖云对象存储的目前 Bucket 名称，默认为当前对象所在的 Bucket | 否 |

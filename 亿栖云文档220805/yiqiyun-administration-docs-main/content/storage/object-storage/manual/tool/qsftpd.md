...
qsftpd 是亿栖云对象存储提供的 FTP Server。用户通过简单的配置，即可通过 FTP 协议使用亿栖云对象存储服务。该工具已在 GitHub 开源，更多详情可参见 [GitHub 项目](https://github.com/qingstor/qsftpd)。
...
```
   ...
   **说明：**
   - `qingstor` 用于标识后续字段为亿栖云对象存储的基本信息。
   - `access_key_id` 与 `secret_access_key` 用于访问亿栖云对象存储的 AK 与 SK，须根据实际情况进行填写。具体可参照 [获取 Access Key](/storage/object-storage/api/practices/signature/#获取-access-key)。
   - `host` 亿栖云对象存储的域名。公有云环境无需修改。
   - `port` 访问亿栖云对象存储的端口号。公有云环境无需修改。
   - `protocol` 访问亿栖云对象存储的协议。公有云环境无需修改。
   - `log_level` 亿栖云对象存储的日志级别。日志内容显示从多到少依次可设置为：debug，info，warn，error，fatal，panic。
   - `bucket_name` 待访问的亿栖云对象存储的 Bucket 名。须根据实际情况进行填写。
   - `zone` 待访问的亿栖云对象存储的 Zone。

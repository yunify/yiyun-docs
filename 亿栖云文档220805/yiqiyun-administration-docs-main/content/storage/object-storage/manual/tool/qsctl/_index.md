...
qsctl 是亿栖云对象存储提供的客户端高级命令行工具，用于管理亿栖云对象存储资源。qsctl 提供了强大的类 UNIX 命令，所有的 qsctl 命令均支持批量操作。
...
亿栖云对象存储建议您升级至 qsctl v2.4.3 版本。详细操作步骤如下：
...
|-|-|
| cat | 输出远程对象内容至标准输出。|
| cp | 复制本地文件至亿栖云对象存储的 Bucket，或从亿栖云对象存储的 Bucket 复制对象至本地。|
| ls | 列出本用户下创建的所有亿栖云对象存储的 Bucket，或列出给定亿栖云对象存储的 Bucket 中给定前缀下的所有对象。|
| mb | 在亿栖云对象存储服务端创建一个新的 Bucket。|
| mv | 移动本地文件至亿栖云对象存储的 Bucket，或移动亿栖云对象存储 Bucket 中的对象至本地。|
| presign | 生成亿栖云对象存储的 Bucket 指定对象的临时下载链接。|
| rb | 从亿栖云对象存储删除 Bucket。|
| rm | 从亿栖云对象存储 Bucket 删除一个对象或给定前缀下的所有对象。|
| stat | 查看一个指定的亿栖云对象存储的对象信息。|
| sync | 同步本地目录和亿栖云对象存储目录。|
| tee | 从标准输入读取内容并上传至亿栖云对象存储的 Bucket。|
 ...
 **说明：**
   ...
   - 命令行中 `qs://mybucket/filename` 为亿栖云对象存储 Bucket 中文件路径，需根据实际情况进行修改。其中若不指定 `filename`，则上传后文件名根据源文件名创建；若指定 `filename`，则将源文件重命名为 `filename` 进行存储。

2. 将本地文件夹上传至亿栖云对象存储的 Bucket 中:

```bash
$ qsctl cp /path/to/folder qs://mybucket/parent/ -r
```

3. 将本地文件夹下的所有文件上传至亿栖云对象存储的 Bucket 中:
```bash
$ qsctl cp /path/to/folder/ qs://mybucket/parent/ -r
```
...
执行如下命令行，从亿栖云对象存储的 Bucket 中下载文件至本地:
 ...
 **说明：**
   - 与上传文件至亿栖云对象存储 Bucket 的区别在于源路径与目标路径的位置不同。
   - 其他用法均与上传文件至亿栖云对象存储 Bucket 保持一致。
...
### 数据同步
`sync` 命令用于本地目录与亿栖云对象存储目录的数据同步。故该操作的源路径与目标路径必须都是目录。由于亿栖云对象存储没有目录概念，所以必须以 `/` 结尾。本地文件夹若为类 Unix 系统，须以 `/` 结尾；若为 Windows 系统请替换为 `\`。若本地文件夹不添加结尾字符，则系统会针对本地文件夹的父目录进行同步操作。
...
从标准输入上传文件至亿栖云对象存储。qsctl 将不会像 Linux tee 命令那样将内容绑定至标准输出。
```bash
...
自 qsctl v2.2.0 版本以来，亿栖云对象存储新加入了交互式的 shell 界面，包含更多的引导和提示信息，推荐新用户使用。若用户需使用 qsctl 开发脚本，建议直接使用前文介绍的命令模式。

亿栖云对象存储将所有命令执行中的交互都移动至 shell 中，包括进度条，删除 bucket 时的确认输入，删除 object 时的确认等。并在命令行直接执行时移除了这些交互效果，以便更好的支持用户使用脚本进行操作。除此之外，用户在终端与 qsctl shell 中输入同样指令所得到的结果是一致的。
...
3. 根据提示，进行相关操作即可。在命令行中，亿栖云对象存储新增了对历史命令和自动补全的支持。详细说明如下：
  - 在行开头可以自动提示补全可用命令；

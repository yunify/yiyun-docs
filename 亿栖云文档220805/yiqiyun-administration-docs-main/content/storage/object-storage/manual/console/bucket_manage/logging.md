...
亿栖云对象存储日志服务可以将 Bucket 的访问日志以 Object 的形式存储至指定 Bucket 下的指定目录。只有 Bucket 的所有者才有权限设置或关闭该功能。

## 操作步骤

1. 进入亿栖云对象存储的主页面，选择待设置日志归档的 Bucket，点击 **右键** > **设置**：
...
- 开启该功能后，归档存储的日志文件会占用存储空间，故，亿栖云对象存储会对其所占存储进行收费。
